...
class FastqDetangler:
    ...
    def detangle(self, input_file: Path, output_prefix: str) -> None:
        ...
        self.logger.info(f"Starting FASTQ detangling process")
        ...
        self.logger.info(f"Processing speed: {len(r1_reads) + len(r2_reads) / total_time:.0f} reads/second")
                ...
                if lines[i].startswith('@'):
                    ...
                    if read_type and i + 3 < len(lines):
                        ...
                        if len(record) == 4 and record[2].startswith('+'):
                            ...
                            self.logger.warning(f"Malformed record at line {i+1}: incomplete or invalid FASTQ format")
                            malformed_records += 1
                            i += 1
                    else:
                        if not read_type:
                            self.logger.warning(f"Invalid read header at line {i+1}: {header}")
                        else:
                            self.logger.warning(f"Incomplete record at line {i+1}: expected 4 lines, got {min(len(lines) - i, 4)}")
                        invalid_reads += 1
                        i += 1
                else:
                    i += 1
            
            self.logger.debug(f"Parsing summary: {valid_reads:,} valid reads, {invalid_reads:,} invalid headers, {malformed_records:,} malformed records")
    ...
    def _identify_missing_pairs(self, r1_reads: Dict[str, List[str]], 
        ...
        self.logger.debug(f"Missing pairs identified: {len(missing_r1):,} R1, {len(missing_r2):,} R2")
